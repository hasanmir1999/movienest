...
export default function AdminPanelEditMoviePageContent({ _id }) {
  ...
  const postData = () => {
    ...
    try {
    dispatch(updateMovie({ id: _id, newData: data }));
      toast.success("Edit movie successfuly!");
      setLoading(false);
    } catch (error) {
      setLoading(false);
  ...
  const loadData = async () => {
    const data = await fetchWithToken(
      `https://movienest.liara.run/api/movie/${_id}`
    );
    ...
    setFormData((prev) => ({
      ...
      big_cover_img: data.big_cover_img || null,
      text: data.text || "",
    }));
  };
  ...
  return (
    <div className="col grow-1 sm:pl-20 sm:pr-14 sm:pb-10">
      <div className="edit-movie bg-black w-full sm:rounded-md sm:border border-gray-400 sm:mt-28 p-5 relative h-auto mt-14">
        <TbTriangleFilled className="absolute text-orange-400 text-2xl -left-3 top-[22px] rotate-90 " />
        <h1 className="text-white text-xl font-bold">Edit movie</h1>
              ...
              <div className="col big w-full lg:w-3/4 p-2">
                <div className="input-container relative w-full h-[380px] bg-gray-800 rounded-md cursor-pointer overflow-hidden group">
                  <input
                    ...
                    accept="image/*"
                  />
                  {formData.big_cover_img && (
                    <Image
                      unoptimized
                      src={
                        formData.big_cover_img instanceof File
                          ? URL.createObjectURL(formData.big_cover_img)
                          : formData.big_cover_img
                      }
                      alt="Cover Preview"
                      className="w-full h-full object-cover"
